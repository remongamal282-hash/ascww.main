...
const industrialWasteRolePdfUrl = `${import.meta.env.BASE_URL}manufactring.pdf`;
...
function IndustrialWasteRolePage() {
  ...
  const handlePdfDownload = async () => {
    ...
    try {
      ...
      link.href = objectUrl;
      link.download = 'industrial-waste-role.pdf';
  ...
  const loadPageRange = async (fromPage: number, toPage: number) => {
    ...
    try {
      for (let pageNumber = fromPage; pageNumber <= toPage; pageNumber += 1) {
        ...
        if (!aliveRef.current || !imageDataUrl) return;
